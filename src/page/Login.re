...
let make = (~setUser) => {
  let (data, setData) =
    React.useState(() => AsyncData.complete(("", "", None)));
  let isBusy = data |> AsyncData.isBusy;
  let (email, password, error) =
    data |> AsyncData.getValue |> Option.getOrElse(("", "", None));
           ...
           | Some(messages) =>
             <ul className="error-messages">
               {messages
                |> Array.map(message =>
                     <li key=message>
                       {Printf.sprintf("email or password %s", message)
                        |> React.string}
                     </li>
                   )
                |> React.array}
            ...
            <fieldset className="form-group">
              <input
                ...
                onChange={event => {
                  ...
                  setData(
                    AsyncData.map(((_email, password, error)) =>
            ...
            <fieldset className="form-group">
              <input
                ...
                onChange={event => {
                  ...
                  setData(
                    AsyncData.map(((email, _password, error)) =>
                      (email, password, error)
                    ),
                  );
                }}
